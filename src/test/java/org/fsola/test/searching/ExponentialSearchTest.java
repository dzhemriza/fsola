 ...
 * org.fsola
 *
 * File Name: ExponentialSearchTest.java
 *
...
import org.fsola.searching.ExponentialSearch;
...
public class ExponentialSearchTest {
    ...
    public void test1() {
        ...
        for (int i = 0; i < a.length; ++i) {
            int result = ExponentialSearch.search(a, a[i]);
            Assert.assertEquals(a[i], a[result]);
        }

        for (int i = 2048; i < 4098; ++i) {
            Assert.assertEquals(-1, ExponentialSearch.search(a, i));
        }
    }
}